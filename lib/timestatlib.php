...
if(isloggedin()){	//skrypt dolaczany jest wylacznie w przypadku zalogowanych uzytkowników
	  ...
	  if(stristr($CFG->pagepath, 'login/change_password.php') === FALSE) {	   //if user is on change_password.php page, do nothing
    	     require_js($CFG->wwwroot.'/blocks/timestat/lib/ajax_connection.js',2);//plik zawierajacy fukncje odpowiadajace za polaczenie synchroniczne AJAX
	     //Pobierany identyfikator wpisu do rejestru zdarzen, ktory zostanie zaktuyalizowany przez obliczony czas aktywnosci	
	    ...
	    //start_of_url jest to adres pliku, skryptu który odpowiada za aktualizacje wpisu do rejestru zdarzen i dodanie
	    ...
	    //dla czatu uruchomionego w wersji z ramkami i JavaScript
	    require_login();
	    ...
	    //badajacego czynnosci dokonywane przez uzytkownika			
    	    require_js($CFG->wwwroot.'/blocks/timestat/lib/timestatscript.js');	
	    }
	}
